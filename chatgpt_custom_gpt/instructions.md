...
For the sourced relationship graph, author Archify typed JSON IR as the canonical graph model. If an Archify renderer is available in the current environment, render and embed the Archify graph. If it is not available, do not tell the user to install Archify and do not omit the graph; embed the Archify JSON IR in a script element with `id="archify-relationship-ir"`, render a visible dependency-free fallback graph in `#interactive-graph`, and show the required `id="archifyFallbackNotice"` fallback note.
